...
# Note, this mode is undocumented.
# Used only to convert ADEXL xml references to png
if ($startExt =~ "xml")
{
  $dir = $endExt;
  $findReplace = 1;
...
else
{
  ...
  foreach my $file (@filesToConvert)
  {
    ...
    system("perl -pi.bak -e 's/\.eps/\.png/g;' $file");
    if ($verbose) {print "Done!\n";}
    if ($clean) {unlink $file.'.bak';}
  }
  print "Find/Replace complete!\n\n";
}
